...
const FlipCard = ({ 
  ...
  a11yScale=1,
  colorTokens,
 }: { flashcard: Flashcard; textData: any,
  a11yScale?: number;
  colorTokens?: any;
  }) => {
  ...
  useEffect(() => {
    // Pop in animation: scale from 0.5 to 1 in 300ms
    Animated.timing(popInAnim, {
      ...
      Animated.loop(
        Animated.sequence([
          Animated.delay(1000), // delay 10 seconds before starting the loop
  ...
  const translateX = slideAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [0, 1], // roughly 1 pixel
  });
      ...
      <View style={styles.cardContainer}>
        ...
        >
          <View style={styles.cardFace}>
            ...
            <Text style={[styles.title, {
                ...
                {textData.getText(flashcard.front)}</Text>
            <Animated.Image
        ...
        >
          <View style={styles.cardFace}>
            ...
            ]}>{textData.getText(flashcard.back)}</Text>
            <Animated.Image
  ...
  return (
    <ScrollView contentContainerStyle={styles.wrapper}>
      {flashcards.map((flashcard: Flashcard) => (
        ...
        key={flashcard.id} flashcard={flashcard} textData={textData} />
      ))}
    </ScrollView>
  );
};
